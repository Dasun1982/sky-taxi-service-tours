...
export default function AirportToHiriketiyaTaxi({ setPage }) {
  return (
    <div className="page colombo-airport-page hiriketiya-taxi-page airport-to-hiriketiya-page">
            ...
            {routeDetails.map((route) => (
              <Reveal className="colombo-airport-route-card" key={route.title}>
                ...
                <div>
                  <h3>{route.title}</h3>
                  <p>{route.text}</p>
                  <a href={buildWhatsAppLink(airportToHiriketiyaMessage(route.title))} target="_blank" rel="noreferrer" aria-label={`Ask route price — ${route.title}`}>
                    Ask route price
                  </a>
                </div>
              </Reveal>
      ...
      <RelatedRoutes destinationId="hiriketiya" pageSource="airport-to-hiriketiya-page" />
    </div>
  );
}
