...
const vehicles = [
  ["toyota-prius", "Affordable private car for a smooth Colombo Airport to Weligama taxi transfer."],
  ["honda-shuttle", "Clean wagon option for airport to Weligama taxi rides with extra room for bags."],
  ["honda-vezel", "Comfortable SUV for couples, families, and surf beach transfers."],
  ["toyota-kdh-van", "Spacious van for families, groups, surfboards, and private transfers to Weligama."],
].map(([id, text]) => {
...
export default function AirportToWeligamaTaxi({ setPage }) {
  return (
    <div className="page colombo-airport-page weligama-taxi-page airport-to-weligama-page">
            ...
            {routeDetails.map((route) => (
              <Reveal className="colombo-airport-route-card" key={route.title}>
                ...
                <div>
                  <h3>{route.title}</h3>
                  <p>{route.text}</p>
                  <a href={buildWhatsAppLink(airportToWeligamaMessage(route.title))} target="_blank" rel="noreferrer" aria-label={`Ask route price — ${route.title}`}>
                    Ask route price
                  </a>
                </div>
              </Reveal>
      ...
      <RelatedRoutes destinationId="weligama" pageSource="airport-to-weligama-page" />
    </div>
  );
}
