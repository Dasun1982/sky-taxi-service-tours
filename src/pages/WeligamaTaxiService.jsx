...
const vehicles = [
  ["toyota-prius", "Affordable private car for Weligama airport transfer, Mirissa trips, and short south coast rides."],
  ["honda-insight", "Clean hybrid option for taxi in Weligama, hotel pickups, and budget-friendly local transfers."],
  ["honda-shuttle", "Luggage-friendly wagon for airport transfers, surf gear, Mirissa routes, Galle trips, and family travel."],
  ["honda-vezel", "Comfortable SUV for private driver Weligama routes to Ella, Yala, Galle, and longer island travel."],
  ["honda-freed", "Family-friendly vehicle for beach transfers, airport pickup, luggage, and custom south coast taxi routes."],
  ["toyota-kdh-van", "Spacious van for groups, families, Colombo airport to Weligama taxi routes, and luggage-heavy trips."],
].map(([id, text]) => {
    ...
    <div className="page colombo-airport-page weligama-taxi-page">
      ...
      >
        <div className="premium-hero-actions">
          <a className="button button--primary" href={buildWhatsAppLink(weligamaTaxiMessage())} target="_blank" rel="noreferrer">
            ...
            {routeCards.map((route) => (
              <Reveal className="colombo-airport-route-card" key={route.title}>
                ...
                <div>
                  <h3>{route.title}</h3>
                  <p>{route.text}</p>
                  <a href={buildWhatsAppLink(weligamaTaxiMessage(route.title))} target="_blank" rel="noreferrer" aria-label={`Ask route price — ${route.title}`}>
                    Ask route price
                  </a>
                </div>
              </Reveal>
            ...
            {surfTransfers.map((transfer) => (
              <Reveal className="colombo-airport-route-card" key={transfer.title}>
                ...
                <div>
                  ...
                  <p>{transfer.text}</p>
                  <a href={buildWhatsAppLink(weligamaTaxiMessage(transfer.title))} target="_blank" rel="noreferrer" aria-label={`Book surf transfer — ${transfer.title}`}>
                    Book surf transfer
                  </a>
                </div>
              </Reveal>
              ...
              <Reveal className="airport-transfer-card" key={vehicle.name}>
                ...
                <div className="airport-transfer-card__body">
                  ...
                  <a
                    ...
                    href={buildWhatsAppLink(weligamaTaxiMessage(`${vehicle.name} Weligama taxi`))}
                    target="_blank"
                    rel="noreferrer"
      ...
      <RelatedTours destinationId="weligama" soft />
      <RelatedDestinations destinationId="weligama" soft={false} />
      <RelatedRoutes destinationId="weligama" pageSource="weligama-taxi-service-page" />
    </div>
  );
}
