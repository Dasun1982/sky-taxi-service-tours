...
const vehicles = [
  ["toyota-prius", "Affordable private car for a smooth Colombo Airport to Sigiriya taxi transfer."],
  ["honda-shuttle", "Clean wagon option for airport to Sigiriya taxi rides with extra room for bags."],
  ["honda-vezel", "Comfortable SUV for couples, families, and Cultural Triangle transfers."],
  ["toyota-kdh-van", "Spacious van for families and groups booking a private transfer to Sigiriya."],
].map(([id, text]) => {
...
function airportToSigiriyaMessage(topic = "Colombo Airport to Sigiriya taxi") {
  return `Hello SKY Taxi Service & Tours, I want to book ${topic}. Flight number: ___ Arrival date/time: ___ Drop-off in Sigiriya: ___ Number of passengers: ___ Luggage: ___`;
}
    ...
    <div className="page colombo-airport-page sigiriya-taxi-page airport-to-sigiriya-page">
      ...
      >
        <div className="premium-hero-actions">
          <a className="button button--primary" href={buildWhatsAppLink(airportToSigiriyaMessage())} target="_blank" rel="noreferrer">
            ...
            {routeDetails.map((route) => (
              <Reveal className="colombo-airport-route-card" key={route.title}>
                ...
                <div>
                  <h3>{route.title}</h3>
                  <p>{route.text}</p>
                  <a href={buildWhatsAppLink(airportToSigiriyaMessage(route.title))} target="_blank" rel="noreferrer" aria-label={`Ask route price — ${route.title}`}>
                    Ask route price
                  </a>
                </div>
              </Reveal>
              ...
              <Reveal className="airport-transfer-card" key={vehicle.name}>
                ...
                <div className="airport-transfer-card__body">
                  ...
                  <a
                    ...
                    href={buildWhatsAppLink(airportToSigiriyaMessage(`${vehicle.name} airport to Sigiriya taxi`))}
                    target="_blank"
                    rel="noreferrer"
      ...
      <RelatedRoutes destinationId="sigiriya" pageSource="airport-to-sigiriya-page" />
    </div>
  );
}
